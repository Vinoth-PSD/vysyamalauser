...
export const AlertSettings = () => {
    ...
    // Toggle all email alerts when heading is clicked
    const toggleAllEmailAlerts = () => {
        const allCurrentlySelected = emailAlerts.every(alert => selectedAlerts[alert.alert_name]);
        const updatedSelectedAlerts = { ...selectedAlerts };
        
        emailAlerts.forEach(alert => {
            updatedSelectedAlerts[alert.alert_name] = !allCurrentlySelected;
        });
        
        setSelectedAlerts(updatedSelectedAlerts);
    };

    // Toggle all SMS alerts when heading is clicked
    const toggleAllSMSAlerts = () => {
        const allCurrentlySelected = smsAlerts.every(alert => selectedAlerts[alert.alert_name]);
        const updatedSelectedAlerts = { ...selectedAlerts };
                ...
                <div>
                    <div className="">
                        <div>
                            {/* Email Alert Section */}
                            <div className="mb-5">
                                {/* Clickable heading for Email Alerts */}
                                <h4 
                                    className="text-[20px] text-primary font-semibold mb-3 max-sm:text-[16px] max-md:mb-1 cursor-pointer hover:underline"
                                    onClick={toggleAllEmailAlerts}
                                >
                                    Email Alert
                                </h4>
                            ...
                            {/* SMS Alert Section */}
                            <div className='mb-5'>
                                {/* Clickable heading for SMS Alerts */}
                                <h4 
                                    className="text-[20px] text-primary font-semibold mb-3 max-sm:text-[16px] max-md:mb-1 cursor-pointer hover:underline"
                                    onClick={toggleAllSMSAlerts}
                                >
                                    SMS Alert
                                </h4>
