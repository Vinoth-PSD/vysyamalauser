...
export const ReUseUpGradePopup: React.FC<ReUseUpGradePopupProbs> = ({ closePopup, text, isAlreadySent }) => {
      ...
      <div className="bg-white rounded-lg p-5 w-3/4 max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">
            {isAlreadySent ? "Status" : "Upgrade Required"}
          </h3>
        </div>
        <p className="mb-4">
          {text}
        ...
        <div className="flex justify-end space-x-4">
          {isAlreadySent ? (
            <button
              onClick={closePopup}
              className="bg-gradient text-white px-6 py-2 rounded-lg font-semibold hover:opacity-90"
            >
              Okay
            </button>
          ) : (
