...
const SearchProfiles = () => {
  ...
  // };
  const SearchProfiles = async (selectedId: number) => {
    try {
      let payload: any = {};

      // decide which parameter to use based on clickedButton
      if (clickedButton === "Profession") {
        payload.profession = selectedId;
      } else if (clickedButton === "State") {
        payload.search_nativestate = selectedId;
      } else if (clickedButton === "City") {
        payload.search_city = selectedId;
      }

      const response = await apiClient.post("/auth/Searchbeforelogin/", payload);
        ...
        >
          <div className="mt-10 space-y-6 max-lg:space-y-5 max-lg:mt-8 max-sm:mt-5 max-sm:space-y-0">
            ...
            >
              ...
              <button
                className={`px-5 py-3 font-semibold rounded-lg max-sm:font-medium max-sm:bg-gray max-sm:text-start max-sm:text-[16px] max-sm:p-3 max-sm:text-primary  max-sm:border-b-[1px] max-sm:rounded-none max-sm:border-footer-text-gray ${clickedButton === "Profession" ? "bg-[#D4D5D9]" : ""
                  }`}
              ...
              <button
                className={`px-5 py-3 font-semibold rounded-lg max-sm:font-medium max-sm:bg-gray max-sm:text-start max-sm:text-[16px] max-sm:p-3 max-sm:text-primary  max-sm:border-b-[1px] max-sm:rounded-none max-sm:border-footer-text-gray ${clickedButton === "City" ? "bg-[#D4D5D9]" : ""
                  }`}
                onClick={() => getCityList()}
              >
                City
              ...
              <button
                className={`px-5 py-3 font-semibold rounded-lg max-sm:font-medium max-sm:bg-gray max-sm:text-start max-sm:text-[16px] max-sm:p-3 max-sm:text-primary  max-sm:border-b-[1px] max-sm:rounded-none max-sm:border-footer-text-gray ${clickedButton === "State" ? "bg-[#D4D5D9]" : ""
                  }`}
