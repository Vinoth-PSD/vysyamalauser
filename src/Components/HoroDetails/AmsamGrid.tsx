...
//   return (
//     <div className="flex  justify-start items-start bg-gray-200 space-x-2 max-lg:space-x-0   max-md:hidden">
//       {/* Labels */}
...
const AmsamGrid: React.FC<AmsamGridProps> = ({ centerLabel, rasiTemp }) => {
  ...
  // Initialize with 12 empty arrays
  const [amsamContents, setAmsamContents] = useState<string[][]>(
          ...
          if (profileData?.amsa_kattam) {
            ...
            if (formattedDatamsamval) {
              try {
                ...
                while (newAmsamContents.length < 12) {
                  newAmsamContents.push([]);
                }
                
                setAmsamContents(newAmsamContents.slice(0, 12));
                ...
                console.error("Error parsing amsam data:", error);
                // Fallback to empty state if parsing fails
                setAmsamContents(Array.from({ length: 12 }, () => []));
  ...
  useEffect(() => {
    const formattedData = formatGridData();
    sessionStorage.setItem("formattedData1", formattedData);
  }, [amsamContents]);
  ...
  return (
    <div className="flex justify-start items-start bg-gray-200 space-x-2 max-lg:space-x-0 max-md:hidden">
      ...
      <div className="flex flex-col gap-2 max-lg:justify-between max-lg:gap-2 max-lg:mb-6 max-md:grid-cols-4 max-md:items-center max-md:justify-between max-md:gap-2 max-md:mb-6 max-sm:grid-cols-2">
        {rasiTemp !== "1" && labels.map((label, _index) => (
          <div
            key={`label-${label.id}`}
            draggable
            onDragStart={(e) => handleDragStart(e, label)}
        ...
        {Array.from({ length: 12 }).map((_, index) => {
          ...
          return (
            <div
              ...
              onDrop={(e) => handleDropRasiBox(e, index)}
              onDragOver={handleDragOver}
              className="relative w-28 h-28 rounded bg-yellow-100 border border-yellow-400 flex flex-col items-start justify-center space-y-2 2xl:w-28 2xl:h-28 max-2xl:w-[92px] max-2xl:h-[92px] max-xl:w-[100px] max-xl:h-[100px] max-lg:w-[120px] max-lg:h-[120px] max-md:h-36 max-md:w-36 max-sm:h-16 max-sm:w-16"
            >
              {amsamContents[index]?.map((label, labelIndex) => (
                <div
